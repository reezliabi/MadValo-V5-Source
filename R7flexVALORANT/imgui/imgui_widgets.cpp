...
bool ImGui::ButtonKU(const char* label, const ImVec2& size_arg)
{
    return ButtonExKey(label, size_arg, 0);
}

bool ImGui::ButtonExKey(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
...
bool ImGui::ButtonEx(const char* label, const ImVec2 & size_arg, ImGuiButtonFlags flags)
    {
        ...
        if (col == ImColor(8, 16, 27, 255))
        {
            RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);

        }

        if (col == ImColor(8, 16, 27, 254))
        {
            RenderTextClippedds(bb.Min + style.FramePadding + ImVec2(12,0), bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
        }
...
bool ImGui::ButtonExsac(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::ButtonExn(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::Buttonsac(const char* label, const ImVec2& size_arg)
{
    return ButtonExsac("ASD", size_arg, 0);
}



bool ImGui::sukar(const char* label, const ImVec2& size_arg)
{
    return Sukarr(label, size_arg, 0);
}

bool ImGui::Buttonn(const char* label, const ImVec2& size_arg)
{
    return ButtonExn(label, size_arg, 0);
}


bool ImGui::ButtonExit(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, 500.0f);

    if (col == GetColorU32(ImGuiCol_ButtonHovered))
    {
        RenderTextClippedbb(bb.Min - ImVec2(1, 5) + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
    }
    else
    {
        RenderTextClipped(bb.Min - ImVec2(1, 5) + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
    }
...
bool ImGui::ButtonExitt(const char* label, const ImVec2& size_arg)
{
    return ButtonExit(label, size_arg, ImGuiButtonFlags_None);
}


bool ImGui::ButtonExitn(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (col == GetColorU32(ImGuiCol_ButtonHovered))
    {
        RenderTextClippedbb(bb.Min - ImVec2(1, 2) + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
    }
    else
    {
        RenderTextClipped(bb.Min - ImVec2(1, 2) + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
    }
...
bool ImGui::ButtonExittn(const char* label, const ImVec2& size_arg)
{
    return ButtonExitn(label, size_arg, ImGuiButtonFlags_None);
}


bool ImGui::ButtonIcon(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
    RenderTextClipped(bb.Min + ImVec2(0, 2) + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::ButtonIconn(const char* label, const ImVec2& size_arg)
{
    return ButtonIcon(label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true) - ImVec2(205, 0);
    const float square_sz = GetFrameHeight();
    ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(square_sz + 10 + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    const ImRect check_bb(pos, pos + ImVec2(square_sz + 15, square_sz - 5));
    ...
    ImVec2 midTop(check_bb.GetCenter().x + 0.5f, check_bb.GetTL().y + 0.5f);
    ImVec2 midBottom(check_bb.GetCenter().x + 0.5f, check_bb.GetBL().y + 0.5f);
    ImVec2 UpperLeft(check_bb.GetTL().x + 1.5f, check_bb.GetTL().y + 0.5f);
    ImVec2 UpperRight(midTop.x + 0.5f, check_bb.GetTR().y + 0.5f);
    ImVec2 BottomLeft(check_bb.GetBL().x + 1.5f, check_bb.GetBL().y + 0.5f);
    ImVec2 BottomRight(midTop.x + 0.5f, check_bb.GetBR().y + 0.5f);
    ImRect FirstHalf(UpperLeft, BottomRight);
    ImColor Blueg2 = ImColor(3, 168, 245, 250);
    ImColor Soluk = ImColor(153, 176, 189, 255);
    ImColor Soluk2 = ImColor(3, 5, 13, 255);
    ImColor Aktif = ImColor(3, 23, 46, 255);

    //Sol Slide
    //window->DrawList->AddRectFilled(check_bb.Min , check_bb.Max, col, 50);
    //window->DrawList->AddLine(midTop, midBottom, col, 2);

    window->DrawList->AddRectFilled(check_bb.Min - ImVec2(2, 2), check_bb.Max + ImVec2(-7, -2), Soluk2, 50.0f);
    window->DrawList->AddCircleFilled(FirstHalf.Min + ImVec2(7.3, 8.5), 8, Soluk, 100);
    ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);
    if (*v) {
        //use mid section initialised above as the (B)TL
        ImVec2 rUpperRight(check_bb.GetTR().x - .5f, check_bb.GetTR().y + .5f);
        ImVec2 rBottomRight(check_bb.GetBR().x - .5f, check_bb.GetBR().y + .5f);
        ImRect SecondHalf(midTop, rBottomRight);
        window->DrawList->AddRectFilled(check_bb.Min - ImVec2(2, 2), check_bb.Max + ImVec2(-7, -2), Aktif, 50.0f);
        //Yeþil kýsým
        ImColor Blueg = ImColor(3, 168, 245, 255);
        window->DrawList->AddCircleFilled(SecondHalf.Min + ImVec2(3, 8.5), 8, Blueg, 100);
    }
...
bool ImGui::Sukarr(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Render
    const ImU32 colt = GetColorU32(ImGuiCol_Anill);
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_Anill : hovered ? ImGuiCol_Anil : ImGuiCol_Anilll);;
    const ImU32 cols = GetColorU32((held && hovered) ? ImGuiCol_colss : hovered ? ImGuiCol_cols : ImGuiCol_colsss);;
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min - ImVec2(1, 1), bb.Max + ImVec2(1, 1), ImColor(2, 19, 39, 255), true, 7.0f);
    RenderFrame(bb.Min, bb.Max, col, true, 7.0f);
    window->DrawList->AddText(nullptr, 13.0f, bb.Min + ImVec2(2, 1), ImColor(cols), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float w = CalcItemWidth();
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w - 10, label_size.y - 4 + style.FramePadding.y * 2.0f));
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(value_x2 + 23, frame_bb.Max.y + 1), frame_col, style.FrameRounding, 15);

    RenderFrameBorder(frame_bb.Min, frame_bb.Max, style.FrameRounding);
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(frame_bb.Min + style.FramePadding, ImVec2(value_x2, frame_bb.Max.y), preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Max.x - 108 + style.ItemInnerSpacing.x, frame_bb.Min.y - 2 + style.FramePadding.y), label);
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            SetNextWindowPos(pos + ImVec2(400-55 -632,-31+255-257));
        }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 13), frame_bb.Max - ImVec2(0, 10), ImColor(2, 19, 39, 255), true, g.Style.FrameRounding);
    ...
    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 13), grab_bb.Max - ImVec2(0, 8), ImColor(1, 76, 114, 255), true, g.Style.FrameRounding);



    const bool Down = (hovered && g.IO.MouseDown[0]);

    ImColor Blueg3 = ImColor(3, 23, 46, 150);

    if (Down)
    {
        window->DrawList->AddCircleFilled(grab_bb.Min + ImVec2(3, 12), 10, Blueg3, 100);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        ...
        window->DrawList->AddCircleFilled(grab_bb.Min + ImVec2(3, 12), 8, GetColorU32(ImGuiCol_SliderGrab), 100);
